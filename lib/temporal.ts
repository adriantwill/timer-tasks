...
export function isNewWeek(lastUpdated: string): boolean {
  try {
    const last = Temporal.Instant.from(lastUpdated).toZonedDateTimeISO(Temporal.Now.timeZoneId()).toPlainDate();
    const now = Temporal.Now.plainDateISO();
    
    return last.year !== now.year || last.weekOfYear !== now.weekOfYear;
  } catch (e) {
    return false;
  }
}
...
export function getNextWeekStart(): string {
  ...
  // Format as "Mon, Jan 1"
  return nextMonday.toLocaleString('en-US', { weekday: 'short', month: 'short', day: 'numeric' });
}
